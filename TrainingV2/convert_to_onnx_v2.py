...
config: DeepSignConfigV2 = checkpoint["config"]
model = DeepSignV2(config).to(DEVICE)
...
onnx_program = torch.onnx.export(
    model,
    (input_tensor, hn_tensor, cn_tensor),
    onnx_path,
    verbose=False,
    opset_version=17,
    input_names=["input", "hn", "cn"],
    output_names=["output"],
    dynamic_axes={
        "input": {1: "sequence"},
    },
