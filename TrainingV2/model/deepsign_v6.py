...
class DeepSignV6(nn.Module):
    def __init__(self, config: DeepSignConfigV6):
        super(DeepSignV6, self).__init__()
        self.config = config
        ...
        self.label_linear = nn.Linear(
            label_rnn_output_size + config.feature_lstm_size,
    ...
    def forward(
        ...
        features_output, _ = self.features_rnn(input, hidden_features)
        features_output = (
            features_output[:, -1, :] if is_batched else features_output[-1]
        )

        handshape_output = self.handshape_linear_dropout(features_output)
        handshape_output_mid = self.handshape_linear(handshape_output)
        handshape_output = self.handshape_dropout(handshape_output_mid)
        ...
        orientation_output_mid = self.orientation_linear(orientation_output)
        orientation_output = self.orientation_dropout(orientation_output_mid)
        ...
        movement_output_mid = self.movement_linear(movement_output)
        movement_output = self.movement_dropout(movement_output_mid)
        movement_output = self.movement_output(movement_output)

        location_output = self.location_linear_dropout(features_output)
        location_output_mid = self.location_linear(location_output)
        location_output = self.location_dropout(location_output_mid)
        location_output = self.location_output(location_output)

        hands_output = self.hands_linear_dropout(features_output)
        hands_output_mid = self.hands_linear(hands_output)
        hands_output = self.hands_dropout(hands_output_mid)
        hands_output = self.hands_output(hands_output)

        features_output = torch.stack(
            [
                handshape_output_mid,
                orientation_output_mid,
                movement_output_mid,
                location_output_mid,
                hands_output_mid,
            ]
        )
        features_output = torch.sum(features_output, dim=0)

        label_output, _ = self.label_rnn(input, hidden_label)
        label_output = label_output[:, -1, :] if is_batched else label_output[-1]
